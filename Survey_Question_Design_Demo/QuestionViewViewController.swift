...
class QuestionViewViewController: UIViewController, UITableViewDelegate {
    ...
    // Remembers the last selected cell, initialized to nil value.
    var lastSelectedCellIndex : Int?
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if (tableView.cellForRowAtIndexPath(indexPath)?.accessoryType == UITableViewCellAccessoryType.Checkmark){
            ...
            if let lastIndex = lastSelectedCellIndex {
                ...
                let lastSelectedIndexPath : NSIndexPath = NSIndexPath(forRow: lastIndex, inSection: 0)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        let selectedRowIndex = self.answerTable.indexPathForSelectedRow
