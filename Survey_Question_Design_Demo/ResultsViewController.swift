...
class ResultsViewController: UIViewController {
    ...
    @IBAction func changeTitles(sender: AnyObject) {
        ...
        let nextTitleIndex = incrementTitleIndex(titleOptionIndex)
        let currentTitle = titleOptions[titleOptionIndex]
    ...
    override func viewDidLoad() {
        ...
        tapRec.addTarget(self, action: #selector(ResultsViewController.tappedView(_:)))
    ...
    func addAllValues(pLayer : PieLayer){
        for index in 0 ..< answers.count{
            let elem : PieElement = PieElement(value: answerValues[index], color: getRandomColor())
...
//    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        let vc : QuestionViewViewController = segue.destinationViewController as! QuestionViewViewController
    ...
    func tappedView(tap : UITapGestureRecognizer){
        ...
        if let tappedElem : PieElement = self.pieLayer.pieElemInPoint(pos) {
            let tappedElemIndex = (self.pieLayer.values as! [PieElement]).indexOf(tappedElem) as Int!
            if let index = lastTappedElemIndex{ // If not the first tap...
                let lastElem : PieElement = pieLayer.values[index] as! PieElement
